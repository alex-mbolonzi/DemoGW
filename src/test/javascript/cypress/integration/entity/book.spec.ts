...
describe('Book e2e test', () => {
  ...
  describe('new Book page', () => {
    ...
    it('should create an instance of Book', () => {
      cy.get(`[data-cy="title"]`).type('Oregon zero').should('have.value', 'Oregon zero');

      cy.get(`[data-cy="description"]`).type('hardware redundant').should('have.value', 'hardware redundant');

      cy.get(`[data-cy="publicationDate"]`).type('2022-05-20').should('have.value', '2022-05-20');

      cy.get(`[data-cy="price"]`).type('82240').should('have.value', '82240');
