package co.ars.gw.web.rest;

import static co.ars.gw.web.rest.TestUtil.sameNumber;
...
import co.ars.gw.IntegrationTest;
import co.ars.gw.domain.Author;
import co.ars.gw.domain.Book;
import co.ars.gw.repository.BookRepository;
import co.ars.gw.service.BookService;
import co.ars.gw.service.criteria.BookCriteria;
import co.ars.gw.service.dto.BookDTO;
import co.ars.gw.service.mapper.BookMapper;
...
class BookResourceIT {
    ...
    @Test
    @Transactional
    void getAllBooksByAuthorBookIsEqualToSomething() throws Exception {
        // Initialize the database
        bookRepository.saveAndFlush(book);
        Author authorBook;
        if (TestUtil.findAll(em, Author.class).isEmpty()) {
            authorBook = AuthorResourceIT.createEntity(em);
            em.persist(authorBook);
            em.flush();
        } else {
            authorBook = TestUtil.findAll(em, Author.class).get(0);
        }
        em.persist(authorBook);
        em.flush();
        book.setAuthorBook(authorBook);
        bookRepository.saveAndFlush(book);
        Long authorBookId = authorBook.getId();

        // Get all the bookList where authorBook equals to authorBookId
        defaultBookShouldBeFound("authorBookId.equals=" + authorBookId);

        // Get all the bookList where authorBook equals to (authorBookId + 1)
        defaultBookShouldNotBeFound("authorBookId.equals=" + (authorBookId + 1));
    }
